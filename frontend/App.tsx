          ...
          <main className="relative z-10 pt-[60px] md:pt-0">
            ...
            {currentView === 'favorites' && (
              <div className="max-w-7xl mx-auto px-4 py-12 md:py-32 animate-fade-in relative z-10 pb-32 md:pb-0">
                ...
                <div className="mb-8"><div className="flex justify-end"><button onClick={handleCreateCollection} className="rounded-full border border-orange-500/30 bg-orange-500/10 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-orange-300 hover:bg-orange-500/20">+ New Collection</button></div>{collections.length > 0 && <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">{collections.map(collection => <div key={collection._id || collection.id} className="glass-card rounded-2xl border border-white/10 p-4"><button onClick={() => setOpenCollection(collection)} className="w-full text-left"><span className="block truncate text-sm font-bold text-white">{collection.name}</span><span className="mt-1 block text-[10px] text-gray-500">{collection.projects?.length || 0} projects · Open</span></button><div className="mt-3 flex gap-3"><button aria-label="Rename collection" onClick={() => handleRenameCollection(collection)} className="text-gray-500 hover:text-orange-400"><Settings size={13}/></button><button aria-label="Delete collection" onClick={() => handleDeleteCollection(collection)} className="text-gray-500 hover:text-red-400"><Trash2 size={13}/></button></div></div>)}</div>}</div>
